...
MYSQL * conn;                   //Establishing a SQL connection
MYSQL_RES *res_set;             //Pointer to the result set form sql 
MYSQL_ROW row;

stringstream stmt;              //Used to construct SQL statements dynamically.
const char * q;
string query;
...
class books {
    ...
    void add(){
        ...
        if (!(res_set))
            ...
            cout << endl << endl << "Entry ERROR !" << endl << "Raise help issue in ItsAJ1005 github account" << endl << endl << endl;
    ...
    void update_price(){
		...
		if((row = mysql_fetch_row(res_set)) != NULL){
			...
			cin >> choice;
				...
				if(choice == 'y' || choice =='Y'){
					...
					if(!(res_set)){
						cout<<endl<<endl<<"Book Price Updates Successfully "<< endl<<endl;
					}
					else{cout << endl << endl << "Entry ERROR !" << endl << "Please raise a issue on ItsAJ1005 on GitHub." << endl << endl << endl;}
				}else{
		...
		} //Function update price successfully finished.
		...
		void update()
		{
			...
			{
				b_id.push_back(std::stoi(row[0]));
				qty.push_back(std::stoi(row[1]));
			}

			mysql_free_result(res_set);

			for (i = 0; i < (int) b_id.size(); i++)
			{
				stmt.str("");
			...
			std::cout << "The orders received have been updated." << std::endl;
		}
...
class purchases
{
	...
	int qty;
	date dt_ordered;
	int eta;			//eta: for estimated expected delivery
	char received;		// Check(T or C or F) F:Orders not recieved; C:Orders Canncelled; T:Orders Recieved
	int inv;
...
class employees
{
	...
	long int salary;
	string mgr_status;	//check(T or F) T: Do Not have Manager Rights; 
...
void password(){
    ...
    for(int i = 0; i < 4; i++){           //A 4 digit password
        num = num * 10 + (getch() - 48);  // Convert ASCII to integer 
        cout<<"*";                        //Displaying * instead of password for privacy      
    }

    if(num == APP_PASS){
        cout<<endl<<endl<<"Password Matched. Authorization complete..."<<endl<<endl;
        cout<<"Press any key...";
        getch();                          //Creating a pause in program workflow for user to read the res
    }else{
        cout<<endl<<endl<<"Incorrect password! Please try again..."<<endl<<endl;
        cout<<"Press any key...";
        getch();
        exit(1);
    }
...
int main(){
    ...
    if(conn){
        ...
        system("cls");
        cout<<"Error while connecting to database."<<endl<<"Raise help issue in ItsAJ1005 github account"<<endl;
        getch();
    }
